...
typedef union _uvc_c9_control_info {
    char buf[60];
    uint32_t header;
    struct {
        uint32_t header;
        uint16_t layout_src_width;
        uint16_t layout_src_height;
        uint16_t layout_dst_width;
        uint16_t layout_dst_height;
        uint8_t window_num;
    } basic;
    struct {
        uint32_t header;
...
struct uvc_c1_control_info {
    uint16_t xu_api_level;   //as number
    char all_firmware_version[20]; //as string
    char cfv_firmware[6]; //as string
};


typedef struct uvc_c4_control_info uvc_c4_control;
typedef struct uvc_c1_control_info uvc_c1_control;
...
enum uvc_pu_ctrl_selector {
    ...
    UVC_PU_CAMERA_SIZE_CONTROL = 0x14,        // XXX
};
...
//定位裁剪----------------------------------------------------------------------
uvc_error_t uvc_get_clip_location(uvc_device_handle_t *devh, uint8_t cur[],
                                  enum uvc_req_code req_code);

//定位裁剪----------------------------------------------------------------------
uvc_error_t uvc_get_clip_location1(uvc_device_handle_t *devh, uvc_c9_control_info uvcC9ControlInfo,
                                   enum uvc_req_code req_code);
