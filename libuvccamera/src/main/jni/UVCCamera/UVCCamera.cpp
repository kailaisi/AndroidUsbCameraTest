...
void UVCCamera::clearCameraParams() {
    ...
    mCameraSizeCur.min = mCameraSizeCur.max = mCameraSizeCur.def = 0;
}
...
//======================================================================
// 明るさ
int UVCCamera::updateCameraSizeCurLimit(int &min, int &max, int &def) {
    ENTER();
    int ret = UVC_ERROR_IO;
    if (mPUSupports & PU_BRIGHTNESS) {
//        UPDATE_CTRL_VALUES(mCameraSizeCur, uvc_get_resolution);
    }
    RETURN(ret, int);
}
...
int UVCCamera::setBrightness(int brightness) {
    ...
    int ret = UVC_ERROR_IO;
    if (mPUSupports & PU_BRIGHTNESS) {
        ret = internalSetCtrlValue(mCameraSizeCur, brightness, uvc_get_brightness,
                                   uvc_set_brightness);
    }
    LOGI("bright = %d", ret);
    RETURN(ret, int);
...
int UVCCamera::setResolution(uvc_c4_control *value) {
    ENTER();
    int ret = UVC_ERROR_IO;
    ret = uvc_set_resolution(mDeviceHandle, value);
    RETURN(ret, int);
}

int UVCCamera::getResolution(uint16_t *value) {
    ENTER();
    int ret = uvc_get_resolution(mDeviceHandle, value, UVC_GET_CUR);
    RETURN(ret, int);
}


int UVCCamera::getClipLocation(uint8_t value[]) {
    ENTER();

    int ret = uvc_get_clip_location(mDeviceHandle, value, UVC_GET_CUR);

    LOGI("getClipLocation:%d", ret);

    RETURN(ret, int);
}

int UVCCamera::setClipLocation(uvc_c9_control_info uvcDeviceInfo) {
    ENTER();
    int ret = -1;
    ret = uvc_set_clip_location(mDeviceHandle, uvcDeviceInfo, UVC_SET_CUR);

    RETURN(ret, int);
}

int UVCCamera::getClipLocation1(uvc_c9_control_info uvcDeviceInfo) {
    ENTER();
    int ret = -1;
    ret = uvc_get_clip_location1(mDeviceHandle, uvcDeviceInfo, UVC_GET_CUR);
    RETURN(ret, int);
}

int UVCCamera::getCtrlC1(uvc_c1_control *uvcDeviceInfo) {
    ENTER();
    int ret = -1;
    ret = uvc_ctrl_c1(mDeviceHandle, uvcDeviceInfo, UVC_GET_CUR);
    RETURN(ret, int);
}
