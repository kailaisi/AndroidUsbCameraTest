...
class UVCCamera {
    ...
    control_value_t mCameraSizeCur;
...
/*	int updateFocusSimpleLimit(int &min, int &max, int &def);
    ...
    int updateCameraSizeCurLimit(int &min, int &max, int &def);

    int setResolution(uvc_c4_control *value);

    int getResolution(uint16_t *value);

    int getClipLocation(uint8_t value[]);


    int setClipLocation(uvc_c9_control_info value);

    int getClipLocation1(uvc_c9_control_info uvcDeviceInfo);

    int getCtrlC1(uvc_c1_control *uvcDeviceInfo);
};
