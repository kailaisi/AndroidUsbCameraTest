...
int UVCPreview::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode,
    ...
    if ((requestWidth != width) || (requestHeight != height) || (requestMode != mode)) {
        ...
        result = uvc_get_stream_ctrl_format_size_fps(mDeviceHandle, &ctrl,
                                                     requestMode == 0 ? UVC_FRAME_FORMAT_YUYV :
                                                     requestMode == 1 ? UVC_FRAME_FORMAT_MJPEG
...
void UVCPreview::uvc_preview_frame_callback(uvc_frame_t *frame, void *vptr_args) {
    ...
    if (preview->mFrameCallbackObj) {
        ...
        env->CallVoidMethod(preview->mFrameCallbackObj, preview->iframecallback_fields.onFrame,
                            buf);
        env->ExceptionClear();
    ...
    if (LIKELY(frame->frame_format == UVC_FRAME_FORMAT_H264)) {
        if (preview->mFrameCallbackObj) {
            jobject buf = env->NewDirectByteBuffer(frame->data, frame->actual_bytes);
            env->CallVoidMethod(preview->mFrameCallbackObj, preview->iframecallback_fields.onFrame,
                                buf);
    ...
    if (LIKELY(!result)) {
        ...
        if (LIKELY(!result)) {
            ...
            LOGI("frameSize=(%d,%d)@%s", frameWidth, frameHeight,
                 (requestMode == 0 ? "YUYV" : requestMode == 1 ? "MJPEG" : "H264"));
            pthread_mutex_lock(&preview_mutex);
            if (LIKELY(mPreviewWindow)) {
                ANativeWindow_setBuffersGeometry(mPreviewWindow,
                                                 frameWidth, frameHeight, previewFormat);
