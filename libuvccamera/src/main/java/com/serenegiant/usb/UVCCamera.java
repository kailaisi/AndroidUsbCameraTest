...
public class UVCCamera {
    ...
    public static int makeFourcc(char paramChar1, char paramChar2, char paramChar3, char paramChar4) {
        return paramChar1 << '\030' & 0xFF000000 | paramChar2 << '\020' & 0xFF0000 | paramChar3 << '\b' & 0xFF00 | paramChar4 & 0xFF;
    }
    ...
    protected int mCurrentFrameFormat = FRAME_FORMAT_H264;
    ...
    protected int mCameraSizeCurMin, mCameraSizeCurMax, mCameraSizeCurDef;
    // until here
...
//================================================================================
     ...
     * @return
     */
    public synchronized int setResolution(final int width, int height) {
        if (mNativePtr != 0) {
            return nativeSetResolution(mNativePtr, width, height);
        }
        return -1;
    }
    ...
    public synchronized int[] getResolution() {
        if (mNativePtr != 0) {
//            nativeUpdateCameraSizeCurLimit(mNativePtr);
            return nativeGetResolution(mNativePtr);
        }
        return new int[8];
    }
     ...
     * @return brightness[%]
     */
    public synchronized short[] getClipLocation() {
        if (mNativePtr != 0) {
//            nativeUpdateCameraSizeCurLimit(mNativePtr);
            return nativeGetClipLocation(mNativePtr);
        }

        return new short[60];
    }

    public synchronized short[] getC1Version() {
        if (mNativePtr != 0) {
//            nativeUpdateCameraSizeCurLimit(mNativePtr);
            return nativeGetC1Version(mNativePtr);
        }
...
//                    nativeUpdateCameraSizeCurLimit(mNativePtr);
                ...
                if (DEBUG) {
                    ...
                    Log.v(TAG, String.format("CameraSizeCur:min=%d,max=%d,def=%d", mCameraSizeCurMin, mCameraSizeCurMax, mCameraSizeCurDef));
                }
            }
        } else {
            mControlSupports = mProcSupports = 0;
    ...
    private final native int nativeUpdateCameraSizeCurLimit(final long id_camera);

    private static final native int nativeSetResolution(final long id_camera, final int width, int height);

    private static final native int[] nativeGetResolution(final long id_camera);

    private static final native short[] nativeGetClipLocation(final long id_camera);

    private static final native short[] nativeGetC1Version(final long id_camera);
...
//**********************************************************************
    ...
    private static final native int nativeSetCameraCur(final long id_camera);

    private static final native int nativeSetClipLocation(final long id_camera);

    private static final native int nativeSetClipLocationCameraIndex_360(final long id_camera, final int cameraIndex);

    private static final native int nativeSetClipLocationGrid(final long id_camera);

    private static final native int nativeSetClipMode(final long id_camera, final int mode);

    private static final native int nativeSetClipLocation360(final long id_camera);

    private static final native int nativeSetClipLocationSpeaker360(final long id_camera, final int speakerX);

    private static final native int nativeSetClipLocationSpeaker_360(final long id_camera, final int speakerX);

    private static final native int nativeSetClipLocation_PIP(final long id_camera, final int speakerX, int stableX, int width);

    private static final native int nativeSetClipLocationStable_360(final long id_camera, int stableX);

    private static final native int nativeSetClipLocationStable(final long id_camera, int stableX);

    private static final native int nativeSetClipLocationScroll_360(final long id_camera, int stableX);

    private static final native int nativeSetDensity(final long id_camera, int srcWidth, int srcHeight, int dstWidth, int dstHeight);

    private static final native int nativeSetClipLocationGridByCenterX(final long id_camera, int index0, int index1, int index2, int index3);

    private static final native int nativeGetC9Mode(final long id_camera);
     ...
     * @return
     */
    public int setClipLocationSpeaker_360(int cameraIndex) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationCameraIndex_360(mNativePtr, cameraIndex);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationCameraIndex_360(int cameraIndex) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationCameraIndex_360(mNativePtr, cameraIndex);
        }
        return -1;
     ...
     * @return
     */
    public int setClipLocationGrid() {
        if (mNativePtr != 0) {
            return nativeSetClipLocationGrid(mNativePtr);
        }
        return -1;
    }
     ...
     * @return
     */
    public int[] getLocation(int micAngle, int dAngle) {
        int orgAngle = micAngle;
        micAngle += dAngle;
        micAngle %= 360;
        int[] ints = new int[2];
        short[] cur = new short[]{3840, 640};
        short i = cur[0];
        double radius = (i / Math.PI / 2);
        double v = micAngle * Math.PI * radius / 180;
        ints[0] = (int) v;
        ints[1] = cur[1];
     ...
     * @return
     */
    public int setClipLocationGridByCenterX() {
        if (mNativePtr != 0) {
            return setClipLocationGridByCenterX(270, 180, 90, 0, 180);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationHalf() {
        if (mNativePtr != 0) {
            return setClipLocationGridByCenterX(270, 0, 90, 180, 0);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationGridByCenterX(int angle1, int angle2, int angle3, int angle4, int dangle) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationGridByCenterX(mNativePtr, getLocation(angle1, dangle)[0],
                    getLocation(angle2, dangle)[0],
                    getLocation(angle3, dangle)[0],
                    getLocation(angle4, dangle)[0]);
        }
        return -1;
    }

    public int setClipLocation360() {
        if (mNativePtr != 0) {
            return nativeSetClipLocation360(mNativePtr);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationSpeaker360(int speakerX) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationSpeaker360(mNativePtr, speakerX);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationSpeaker_360_(int speakerX) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationSpeaker_360(mNativePtr, speakerX);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationStable_360(int stableX) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationStable_360(mNativePtr, stableX);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationStable(int stableX) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationStable(mNativePtr, stableX);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipLocationScroll_360(int stableX) {
        if (mNativePtr != 0) {
            return nativeSetClipLocationScroll_360(mNativePtr, stableX);
        }
        return -1;
    }
     ...
     *
     * @param x       mic对应位置
     * @param stableX 固定画面位置
     * @param width   小画面大小
     * @return
     */
    public int setClipLocation_PIP(int x, int stableX, int width) {
        if (mNativePtr != 0) {
            return nativeSetClipLocation_PIP(mNativePtr, x, stableX, width);
        }
        return -1;
    }
     ...
     * @return
     */
    public int setClipMode(int mode) {
        if (mNativePtr != 0) {
            return nativeSetClipMode(mNativePtr, mode);
        }
        return -1;
    }

    public int setDensity(int srcWidth, int srcHeight, int dstWidth, int dstHeight) {
        if (mNativePtr != 0) {
            return nativeSetDensity(mNativePtr, srcWidth, srcHeight, dstWidth, dstHeight);
        }
        return -1;
    }

    public int getC9Mode() {
        if (mNativePtr != 0) {
            return nativeGetC9Mode(mNativePtr);
        }
        return 0;
    }
}
