...
public final class DeviceFilter {
	...
	public DeviceFilter(final int vid, final int pid, final int clasz, final int subclass,
			final int protocol, final String manufacturer, final String product, final String serialNum, final String isExclude) {
		mVendorId = vid;
		mProductId = pid;
		mClass = clasz;
		mSubclass = subclass;
...
/*		Log.i(TAG, String.format("vendorId=0x%04x,productId=0x%04x,class=0x%02x,subclass=0x%02x,protocol=0x%02x",
	...
	public DeviceFilter(final UsbDevice device, final String isExclude) {
        ...
        while (eventType != XmlPullParser.END_DOCUMENT) {
        	...
        	if (!TextUtils.isEmpty(tag) && (tag.equalsIgnoreCase("usb-device"))) {
        		if (eventType == XmlPullParser.START_TAG) {
					...
					exclude = getAttributeString(context, parser, null, "exclude", "false");
...
/*		if (mManufacturerName != null && device.getManufacturerName() == null)
	...
	public boolean isExclude(final UsbDevice device) {
		return isExclude.equals("true") && matches(device);
	}
